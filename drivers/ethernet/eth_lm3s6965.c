...
#define SYS_LOG_DOMAIN "dev/eth_lm3s6965"
...
#include "eth_lm3s6965_priv.h"

static void eth_lm3s6965_assign_mac(struct device *dev)
{
	u32_t value = 0x0;

	value |= CONFIG_ETH_LM3S6965_MAC0;
	value |= CONFIG_ETH_LM3S6965_MAC1 << 8;
	value |= CONFIG_ETH_LM3S6965_MAC2 << 16;
	value |= CONFIG_ETH_LM3S6965_MAC3 << 24;
	sys_write32(value, REG_MACIA0);

	value = 0x0;
	value |= CONFIG_ETH_LM3S6965_MAC4;
	value |= CONFIG_ETH_LM3S6965_MAC5 << 8;
	sys_write32(value, REG_MACIA1);
}

static void eth_lm3s6965_dev_init(struct device *dev)
{
	u32_t value;
...
static int eth_lm3s6965_send(struct net_if *iface, struct net_pkt *pkt)
{
	struct net_eth_hdr *pkt_hdr;
	struct device *dev;
	struct eth_lm3s6965_runtime *dev_data;
	u32_t reg_val;
	int bytes_left;
	u8_t *data_buf;
	...
	bytes_left = net_pkt_get_len(pkt) + net_pkt_ll_reserve(pkt);
	data_buf = net_pkt_ll(pkt);
	reg_val = net_pkt_get_len(pkt);
	reg_val |= ((u32_t)(*data_buf++) << 16);
	reg_val |= ((u32_t)(*data_buf++) << 24);

	/* Send the first word */
	sys_write32(reg_val, REG_MACDATA);
	bytes_left -= 2;

	/* Write the rest of data in net_buf */
	for (bytes_left; bytes_left > 3; bytes_left -= 4) {
		sys_write32(*(u32_t *)data_buf, REG_MACDATA);
		data_buf += 4;
	}
	...
	if (bytes_left > 0) {
		...
		switch (bytes_left) {
		case 3:
			reg_val |= ((u32_t)data_buf[2] << 16);
		case 2:
			reg_val |= ((u32_t)data_buf[1] << 8);
		case 1:
			reg_val |= (u32_t)data_buf[0];
			break;
		default:
			break;
		}
...
void eth_lm3s6965_rx(struct device *dev)
{
	struct net_pkt *pkt = NULL;
	struct eth_lm3s6965_runtime *dev_data = DEV_DATA(dev);
...
static void eth_lm3s6965_init(struct net_if *iface)
{
	struct device *dev;
	struct eth_lm3s6965_config *dev_conf;
	struct eth_lm3s6965_runtime *dev_data;
...
static struct device DEVICE_NAME_GET(eth_lm3s6965);

static void eth_lm3s6965_irq_config(struct device *dev)
{
	ARG_UNUSED(dev);
	/* Enable Interrupt */
	IRQ_CONNECT(IRQ_ETH, 0, rx_isr, DEVICE_GET(eth_lm3s6965), 0);
	irq_enable(IRQ_ETH);
}

static struct eth_lm3s6965_config eth_cfg = {
	.mac_base = ETH_MAC_BASE_ADDR,
	.irq_num = IRQ_ETH,
	.config_func = eth_lm3s6965_irq_config,
};

static struct eth_lm3s6965_runtime eth_data = {
	.mac_addr = {
		CONFIG_ETH_LM3S6965_MAC0,
		CONFIG_ETH_LM3S6965_MAC1,
		CONFIG_ETH_LM3S6965_MAC2,
		CONFIG_ETH_LM3S6965_MAC3,
		CONFIG_ETH_LM3S6965_MAC4,
		CONFIG_ETH_LM3S6965_MAC5
	},
	.tx_err = false,
};

static const struct ethernet_api eth_lm3s6965_apis = {
	.iface_api.init	= eth_lm3s6965_init,
	.iface_api.send = eth_lm3s6965_send,
};

NET_DEVICE_INIT(eth_lm3s6965, CONFIG_ETH_LM3S6965_NAME,
		eth_lm3s6965_dev_init, &eth_data, &eth_cfg,
		CONFIG_ETH_INIT_PRIORITY,
		&eth_lm3s6965_apis, ETHERNET_L2,
